...
export default @Controller('/search') class SearchController {
    ...
    async searchUsers(req, res) {
        ...
        // forcefully restrict output
        if (!pipeline[pipeline.length - 1].$count) {
            pipeline.push({
                $project: {
                    "displayName": "$data.displayName",
                    "publicKeys": "$data.publicKeys",
                    "publicKeyProofs": "$data.publicKeyProofs",
                }
            });
        }
        ...
        ({ id, cursor } = await this.getCursorById(id, async () => {
            ...
            return await db.collection(config.table.accounts).aggregate(pipeline);
        }));
    ...
    cleanCursors() {
        ...
        for (const { id, time } of Object.values(this.cursors)) {
            // after 25 mins, retire this cursor
            if (now - time >= 25 * 60000) {
                this.removeCursorById(id);
            }
        }
    }
