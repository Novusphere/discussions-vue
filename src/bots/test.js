...
async function generateDetails(from, swapAmount, swapFromId, swapToId) {
    ...
    const memo = composeMemo(convertPath, estimate.amount, from);
...
(async function () {
    ...
    const amount = number_to_asset(0.1, { code: `EOS`, precision: 4 });
    const details = await generateDetails(from, amount, `eosio.token-EOS`, `novusphereio-ATMOS`);
